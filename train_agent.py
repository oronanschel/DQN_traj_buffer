...
def load_params(folder_name):
  ...
  tempdir = os.path.join(tempdir,folder_name)
  model_params = pkl.load(open(os.path.join(tempdir, 'model_params.pkl'), 'rb'))
  return model_params
...
class Trainer(object):
  def create_results_file(self):
    ...
    self.res_file = os.path.join(self.mydir, 'train_and_test.txt')
  ...
  def __init__(self, model_params, ale_env, folder_name):
    ...
    if model_params.nn_file is None:
      ...
      tmpdir = os.path.join(os.getcwd(), "models")
      self.mydir = os.path.join(tmpdir, folder_name)

      self.prog_file = os.path.join(self.mydir, 'training_progress.csv')
      self.res_file  = os.path.join(self.mydir, 'train_and_test.txt')
      old_params = load_params(folder_name)

      self.params = old_params
      self.params.rom_path = old_params.rom_path
      self.params.folder_name = folder_name
      self.params.nn_file = model_params.nn_file

    # print 'TUNED VERSION'
    #self.params.optimal_eps = 0.001 # TODO: TUNED VERSION
    #self.params.epsilon_min = 0.01  # TODO: TUNED VERSION

    self.num_of_prime = self.params.num_of_prime
    ...
    # theano.config.print_active_device = True
    # theano.config.device = 'gpu1'
    # theano.config.force_device = ODOrue
    ...
    if folder_name:
      self.params.frame_count = 18*(500000) # TODO: set when cont last epoch*200,000
      self.frame_count = self.params.frame_count
    else:
      self.frame_count = 0.
    self.best_reward = -100.
    self.max_frames_per_game = 18000
    # self.max_frames_per_game = 500
  ...
  def get_action(self, epsilon, x, testing):
    if np.random.rand() < epsilon:
      return np.random.randint(len(self.legal_actions))
    else:
      return self.model.predict_move(np.array(x).reshape(1,4,84,84), testing)

  def save_model(self, total_reward, skip_best=False):
    if total_reward >= self.best_reward and not skip_best:
      self.best_reward = total_reward
      for k in range(self.num_of_prime+1):
        if k==0:
          pkl.dump(self.model.save_params(k), open(os.path.join(self.mydir, 'best_model.pkl'), "w"), protocol=pkl.HIGHEST_PROTOCOL)
        else:
          pkl.dump(self.model.save_params(k), open(os.path.join(self.mydir, 'best_model'+str(k)+'.pkl'), "w"),
                   protocol=pkl.HIGHEST_PROTOCOL)

    for k in range(self.num_of_prime + 1):
      if k == 0:
        pkl.dump(self.model.save_params(k), open(os.path.join(self.mydir, 'last_model.pkl'), "w"), protocol=pkl.HIGHEST_PROTOCOL)
      else:
        pkl.dump(self.model.save_params(k), open(os.path.join(self.mydir, 'last_model'+str(k)+'.pkl'), "w"),
                 protocol=pkl.HIGHEST_PROTOCOL)

    pkl.dump(self.params, open(os.path.join(self.mydir, 'model_params.pkl'), "wb"))
    print "Saved model"
  ...
  def run_testing(self, epoch):
    ...
    while(self.frame_count - original_frame_count < self.params.steps_per_test):
      ...
      if hasattr(self,'res_file'):
        res_file = open(self.res_file, 'a')
        res_file.write("TESTING: %d fps,\t" % fps)
        res_file.write("%d frames,\t" % self.ale.getEpisodeFrameNumber())
        res_file.write("%d points,\t" % reward)
        res_file.write('eps: %.4f,\n' % self.params.optimal_eps)
        res_file.close()
    ...
    mean_q = self.get_mean_q_val()
    self.update_results(epoch+1, mean_reward, mean_q)
    self.save_model(total_reward=mean_reward)

  def train(self):
    cumulative_reward = 0
    counter = 0 # TODO: set when cont last ep train_and_test.txt
    start_epoch = 0 # TODO: set when cont last epoch in training_progress.csv
    frame_count_old =self.frame_count
    for i in range(start_epoch,self.params.epochs):
      ...
      while self.frame_count-start_frames < self.params.steps_per_epoch:
        ...
        res_file = open(self.res_file, 'a')
        res_file.write(("ep %d,\t") % (counter+1))
        print ("ep %d,\t") % (counter+1),

        res_file.write("%d fps,\t" % fps)
        print ("%d fps,\t" % fps),
        ...
        self.ale.reset_game()
        res_file.write('%d points,\t' % total_reward)
        print ('%d points,\t' % total_reward),

        res_file.write('%.1f avg,\t' % (float(cumulative_reward)/(counter+1)))
        ...
        print "FC delta: %d" % (self.frame_count-frame_count_old)
        frame_count_old = self.frame_count
        counter +=1
        res_file.close()
...
class DQN_Trainer(Trainer):
  ...
  def run_training_episode(self, max_steps, testing=False):
    ...
    while not game_over:
      ...
      chosen_action = self.get_action(epsilon, x, testing=testing)
      ...
      if self.frame_count > self.params.replay_start_size and not testing:
        # TODO: un-if if start of a new game (fills the buffer in case of cont game)
        # if self.frame_count-self.params.frame_count > self.params.replay_start_size*7:
...
class Q_Learning(DQN_Trainer):
  def __init__(self, **kwargs):
    ...
    self.model = DeepQNetwork(model_network=model_network, double_q=self.params.double_q,
      ...
      gamma=self.params.discount,num_of_prime=self.params.num_of_prime)
    if self.params.nn_file is not None:
      tempdir = os.path.join(os.getcwd(), "models")
      tempdir = os.path.join(tempdir, self.params.folder_name)
      tempdir = os.path.join(tempdir, self.params.nn_file)

      self.model.load_params(pkl.load(open(tempdir, 'r')),0)
      last_vals = pkl.load(open(tempdir, 'r'))
      for k in range(1,self.num_of_prime+1):
        try:
          path = tempdir.split('.')[0]+str(k)+'.'+tempdir.split('.')[1]
          vals = pkl.load(open(path, 'r'))
          last_vals = vals
        except:
          print 'PARAMS COULDNT BE LOADED K='+str(k)
          vals = last_vals

        self.model.load_params(vals, k)
