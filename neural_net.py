...
class DeepQNetwork():
  def __init__(self, model_network=None,gamma=0.99, learning_method="rmsprop", 
    ...
    scale=255., double_q=False, prioritized_exp_replay=False,num_of_prime=1):
    x = T.ftensor4()
    next_x = T.ftensor4()
    a = T.ivector()
    r = T.fvector()
    ...
    # TODO: check-list when starting a run
    # num_of_prime = 5

    self.use_in_test = True
    self.use_in_train = True
    self.use_as_train_policy = False


    self.Q_model = Model(model_network, input_size=input_size, dnn_type=dnn_type)
    self.Q_prime_model = [Model(model_network, input_size=input_size, dnn_type=dnn_type) for _ in range(num_of_prime)]
    ...
    if double_q:
      ...
      if not self.use_in_train:
        y = r + (T.ones_like(terminal) - terminal) * gamma * T.max(self.Q_prime_model[0].apply(next_x / scale), axis=1)
      else:
        q = 0
        for k in range(0,num_of_prime):
          q+=self.Q_prime_model[k].apply(next_x/scale)
        q = q/num_of_prime
        y = r + (T.ones_like(terminal) - terminal) * gamma * T.max(q, axis=1)


    # for test only
    all_q_vals_avg = 0
    for k in range(0, num_of_prime):
      all_q_vals_avg += self.Q_prime_model[k].apply(next_x / scale)
    ...
    for t, b in zip(self.Q_prime_model[0].params, self.Q_model.params):
      target_updates[t] = b
    for k in range(0,num_of_prime-1):
      for t, b in zip(self.Q_prime_model[k+1].params, self.Q_prime_model[k].params):
        target_updates[t] = b
    ...
    self.pred_score_avg = theano.function([], all_q_vals_avg, givens={x:self.x_shared,next_x:self.x_shared})
  ...
  def predict_move(self, x, testing):
    self.x_shared.set_value(x)
    if testing and self.use_in_test or self.use_as_train_policy:
      return np.argmax(self.pred_score_avg(), axis=1)
    else:
  ...
  def save_params(self,k):
    if k==0:
      ...
      return self.Q_prime_model[k-1].save_params()

  def load_params(self, values,k):
    if k==0:
      self.Q_model.load_params(values)
    else:
      self.Q_prime_model[k-1].load_params(values)
